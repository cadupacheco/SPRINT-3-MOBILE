...
import { Text, Card, Title, Paragraph, Button, Divider, ProgressBar } from 'react-native-paper';
...
type MotorcycleDetailsRouteProp = RouteProp<RootStackParamList, 'MotorcycleDetails'>;
type MotorcycleDetailsNavigationProp = NativeStackNavigationProp<RootStackParamList, 'MotorcycleDetails'>;
...
export default function MotorcycleDetailsScreen() {
  ...
  const handleStatusChange = async (newStatus: 'available' | 'maintenance' | 'rented' | 'out_of_service') => {
    if (!motorcycle) return;

    const success = await actions.updateMotorcycleById(id, { status: newStatus });
    ...
    <ScrollView style={styles.container}>
      <Card style={styles.card}>
        <Card.Content>
          ...
          <Paragraph style={styles.plate}>Placa: {motorcycle.plate}</Paragraph>
      ...
      <Card style={styles.card}>
        <Card.Content>
          ...
          <ProgressBar 
            ...
            color={motorcycle.batteryLevel > 50 ? '#4CAF50' : motorcycle.batteryLevel > 20 ? '#FF9800' : '#F44336'} 
          ...
          <ProgressBar 
            ...
            color={motorcycle.fuelLevel > 50 ? '#4CAF50' : motorcycle.fuelLevel > 20 ? '#FF9800' : '#F44336'} 
            style={styles.progressBar} 
          />
          
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>Quilometragem:</Text>
            <Text style={styles.infoValue}>{motorcycle.mileage?.toLocaleString() || 'N/A'} km</Text>
          </View>
          
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>Localização no Pátio:</Text>
            <Text style={styles.infoValue}>X: {motorcycle.location.x}, Y: {motorcycle.location.y}</Text>
          </View>
          
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>Filial:</Text>
            <Text style={styles.infoValue}>{motorcycle.assignedBranch || 'Não informado'}</Text>
          </View>
          
          <View style={styles.infoRow}>
            <Text style={styles.infoLabel}>Última Atualização:</Text>
            <Text style={styles.infoValue}>
              {motorcycle.lastUpdate ? new Date(motorcycle.lastUpdate).toLocaleString('pt-BR') : 'N/A'}
            </Text>
          </View>
        </Card.Content>
      </Card>
